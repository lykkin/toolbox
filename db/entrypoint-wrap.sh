...
PS="CREATE TABLE IF NOT EXISTS span_collector.to_process (trace_id text, span_id text, parent_id text, name text, start_time double, finish_time double, category text, tags map<text,text>, license_key text, entity_id text, entity_name text, PRIMARY KEY(trace_id, license_key, span_id));"
