...
func NewSpanCollector(session *gocql.Session) func(http.ResponseWriter, *http.Request) {
    return func(w http.ResponseWriter, r *http.Request) {
        ...
        for _, span := range incomingSpans {
            query += "INSERT INTO span_collector.span (name, span_id, trace_id, start_time, finish_time) VALUES (?, ?, ?, ?, ?);"
            values = append(values, span.Name, span.SpanId, span.TraceId, span.StartTime, span.FinishTime)
        }
        query += "APPLY BATCH;"
...
func NewSpanViewer(session *gocql.Session) func(http.ResponseWriter, *http.Request) {
    return func(w http.ResponseWriter, r *http.Request) {
        iter := session.Query("SELECT name, span_id, trace_id, start_time, finish_time FROM span_collector.span;").Iter()
        log.Print(iter)
        spans := make([]Span, 0)
        ...
        for iter.Scan(&name, &spanId, &traceId, &startTime, &finishTime) {
            spans = append(spans, Span{traceId, spanId, name, startTime, finishTime})
        }
